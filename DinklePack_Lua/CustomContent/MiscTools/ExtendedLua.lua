...
local function OnScriptCommand(event, player, command)
    -- split command to get subcommand
    local splitCommand = command:split(" ")

    -- check if .script command is used
    if splitCommand[1] == "script" then
        -- get the targeted creature
        local creature = player:GetSelection()

        -- check if the player has a creature targeted
        if creature == nil or creature:GetTypeId() ~= 3 then -- 3 is the type ID for creatures
            player:SendBroadcastMessage("You must target a creature first!")
            return false
        end

        -- get the creature's script name and script ID
        local scriptName = creature:GetScriptName()
        local scriptId = creature:GetScriptId()

        -- send a message to the player with the script name and script ID
        player:SendBroadcastMessage("The script name of the targeted creature is: " .. scriptName)
        player:SendBroadcastMessage("The script ID of the targeted creature is: " .. scriptId)

        return false -- prevent the command from being handled elsewhere
    end
end
