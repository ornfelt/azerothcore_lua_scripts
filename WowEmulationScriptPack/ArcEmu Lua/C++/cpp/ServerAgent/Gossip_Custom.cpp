...
public:
	void Destroy()
	{
	  ...
	  {
		  GossipMenu *Menu;
		  objmgr.CreateGossipMenuForPlayer(&Menu, pObject->GetGUID(), 3874, Plr); // == SendQuickMenu(3874);
		  

			if(Plr->GetTeam() > 0)  // horde... hacky :D
			{
		  ...
		  if((Plr->GetSession()->CanUseCommand('y')))  // vipek `permission` je 'y' legyen
	  ...
	  void GossipSelectOption(Object* pObject, Player* Plr, uint32 Id, uint32 IntId, const char * Code)
	  {
			...
			int32 newgoldfull = currentcoins + 10000000;	 // 1000 gold = 10,000,000 copper
			...
			case 3:			// arenak
				 ...
				 break;
			case 4:
				 Plr->RemoveAura(15007);  // sickness
				 Plr->RemoveAura(26013);  // deserter
			...
			case 5:
				 ...
				 break;
			case 6:		// VIP
				 objmgr.CreateGossipMenuForPlayer(&Menu, pObject->GetGUID(), 3874, Plr); // elkésziti a "menut"
				 ...
				 Menu->SendTo(Plr); // elkuldi a playernek
				 break;
			case 7:				 // cox
			...
			case 15:
				 ...
				 Plr->BroadcastMessage("%sLevel 80 vagy! Ha a statjaid vagy skilljeid nem emelkedtek meg rendesen akkor nyomj egy reloggot!", MSG_COLOR_GREEN);
				 Plr->SaveToDB(false);
				 GossipHello(pObject, Plr, true);
				 break;
