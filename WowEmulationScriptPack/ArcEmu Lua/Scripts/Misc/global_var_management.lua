...
function setvars(Unit, tblArgs)
	...
	clean_table(0); -- clean any stale entries. 300 seconds = 5 minutes.
	do_garbagecollection(0); -- collect garbage. 900 seconds = 15 minutes.
...
function getvars(Unit)
	...
	-- But will also return calling function line number.
	if (type(Unit) ~= "userdata") then
		print("getvars arg 1 expecting userdata, got "..type(Unit).."->"..tostring(Unit));
		return nil;
	end
	...
	if (instance == "nil") then
		if (_globalvars_[Unit] == nil) then
			print("getvars attempting to get data from nonexistent table entry");
			return nil;
		end
