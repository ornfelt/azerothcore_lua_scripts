...
public:
    ...
    {
        static std::vector<ChatCommand> homeCommandTable =
        {
            { "enter",           rbac::RBAC_PERM_COMMAND_HOME_ENTER,        false, &HandleHomeCommandEnter,       "" },
            { "endurance",       rbac::RBAC_PERM_COMMAND_HOME_ADDNPC,       false, &HandleHomeCommandEndurance,   "" },
            { "addnpc",          rbac::RBAC_PERM_COMMAND_HOME_ADDNPC,       false, &HandleHomeCommandAddNpc,      "" },
            { "delnpc",          rbac::RBAC_PERM_COMMAND_HOME_DELNPC,       false, &HandleHomeCommandDelNpc,      "" },
            { "addgo",           rbac::RBAC_PERM_COMMAND_HOME_ADDGO,        false, &HandleHomeCommandAddGo,       "" },
            { "selgo",           rbac::RBAC_PERM_COMMAND_HOME_SELGO,        false, &HandleHomeCommandSelGo,       "" },
            { "delgo",           rbac::RBAC_PERM_COMMAND_HOME_DELGO,        false, &HandleHomeCommandDelGo,       "" },
            { "fly",             rbac::RBAC_PERM_COMMAND_HOME_FLY,          false, &HandleHomeCommandFly,         "" },
            { "",                rbac::RBAC_PERM_COMMAND_HOME,              false, &HandleHomeCommand,            "" },
        };
        static std::vector<ChatCommand> lootCommandTable =
        {
            { "quest",           rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootQuest,             "" },
            { "empty",           rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootEmpty,             "" },
            { "noequip",         rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootNoequip,           "" },
            { "equip",           rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootEquip,             "" },
            { "gray",            rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootGray,              "" },
            { "white",           rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootWhite,             "" },
            { "green",           rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootGreen,             "" },
            { "blue",            rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootBlue,              "" },
            { "purple",          rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootPurple,            "" },
            { "crafting",        rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootCrafting,          "" },
            { "",                rbac::RBAC_PERM_COMMAND_LOOT,     false, &HandleLootCommand,           "" },
        };
        static std::vector<ChatCommand> xmorphCommandTable =
        {
            { "self",            rbac::RBAC_PERM_COMMAND_XMORPH_SELF,       false, &HandleXmorphSelf,             "" },
            { "pet",             rbac::RBAC_PERM_COMMAND_XMORPH_PET,        false, &HandleXmorphPet,              "" },
            { "companion",       rbac::RBAC_PERM_COMMAND_XMORPH_COMP,       false, &HandleXmorphCompanion,        "" },
            { "mount",           rbac::RBAC_PERM_COMMAND_XMORPH_MOUNT,      false, &HandleXmorphMount,            "" },
            { "",                rbac::RBAC_PERM_COMMAND_XMORPH,            false, &HandleXmorphCommand,          "" },
        };
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "endurance",        rbac::RBAC_PERM_COMMAND_HOME,             false, &HandleEnduranceCommand,        "" },
            ...
            { "events",           rbac::RBAC_PERM_COMMAND_EVENTS,           true,  &HandleEventsCommand,           "" },
            { "home",             rbac::RBAC_PERM_COMMAND_HOME,             true,  nullptr,                        "",  homeCommandTable },
            { "loot",             rbac::RBAC_PERM_COMMAND_LOOT,             true,  nullptr,                        "",  lootCommandTable },
            { "xmog",             rbac::RBAC_PERM_COMMAND_TRANSMOG_XMOG,    true,  &HandleXmogSlotCommand,         "" },
            { "xmorph",           rbac::RBAC_PERM_COMMAND_XMORPH,           true,  nullptr,                        "",  xmorphCommandTable },
            { "xreset",           rbac::RBAC_PERM_COMMAND_XRESET,           true,  &HandleXresetCommand,           "" },
        };
        return commandTable;
    }

    static bool HandleEventsCommand(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    RBAC_ShowEvents(player, 0);
                    player->BroadcastMessage("Also try command: .events next");
                    return true;
                }
                else
                {
                    ...
                    if (argstr == "next")
                    {
                        RBAC_ShowEvents(player, 1);
                        return true;
                    }
                    player->BroadcastMessage("Syntax: .events OR .events next");
                    return false;
                }
            }
        }
    ...
    static bool HandleHomeCommand(ChatHandler* handler, char const* /*args*/) {
        handler->ShowHelpForCommand(handler->getCommandTable(), "home");
        return true;
    }

    static bool HandleHomeCommandEnter(ChatHandler* handler, char const* args) {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    TeleportToPersonalInstanceMapWithChecks(player, 615, true);
                    return false;
                }
                else
                {
    ...
    static bool HandleHomeCommandEndurance(ChatHandler* handler, char const* /*args*/) {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                    TeleportToPersonalInstanceMapWithChecks(player, ENDURANCE_MAP, true);
            }
        }
        return true;
    }

    static bool HandleEnduranceCommand(ChatHandler* handler, char const* /*args*/) { //TODO with mapId
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_StartEndurance(player);
            }
        }
        return true;
    }

    static bool HandleHomeCommandAddNpc(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .home addnpc ID");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_HomeCommands(player, 0, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleHomeCommandDelNpc(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_HomeCommands(player, 1, "");
            }
    ...
    static bool HandleHomeCommandAddGo(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .home addgo ID");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_HomeCommands(player, 2, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleHomeCommandSelGo(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_HomeCommands(player, 3, "");
            }
    ...
    static bool HandleHomeCommandDelGo(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_HomeCommands(player, 4, "");
            }

        }
        return true;
    }
    static bool HandleHomeCommandFly(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .home fly on/off");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_HomeCommands(player, 5, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleXmogCommand(ChatHandler* handler, char const* /*args*/) {
        handler->ShowHelpForCommand(handler->getCommandTable(), "xmog");
        return true;
    }

    static bool HandleXmogSlotCommand(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .xmog SLOT ItemID");
                    return false;
                }
                else
                {
                    ...
                    char const* nameorid = strtok(nullptr, " ");
                    if (!nameorid)
                        return false;


                    RBAC_Transmog_Xmog(player, slot, nameorid);
                    return true;
                }
            }
        }
    ...
    static bool HandleXmorphCommand(ChatHandler* handler, char const* /*args*/) {
        handler->ShowHelpForCommand(handler->getCommandTable(), "xmorph");
        return true;
    }

    static bool HandleXmorphSelf(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .xmorph self ModelID");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_XMorph(player, 0, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleXmorphPet(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .xmorph pet ModelID");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_XMorph(player, 1, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleXmorphCompanion(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .xmorph companion ModelID");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_XMorph(player, 2, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleXmorphMount(ChatHandler* handler, char const* args)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (!*args)
                {
                    player->BroadcastMessage("Syntax: .xmorph mount ModelID");
                    return false;
                }
                else
                {
                    std::string argstr = (char*)args;
                    RBAC_XMorph(player, 3, (char*)args);
                    return true;
                }
            }
        }
    ...
    static bool HandleLootCommand(ChatHandler* handler, char const* /*args*/) {
        handler->ShowHelpForCommand(handler->getCommandTable(), "loot");
        return true;
    }

    static bool HandleLootQuest(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 0);
            }
        }
        return true;
    }
    static bool HandleLootEmpty(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 1);
            }
        }
        return true;
    }
    static bool HandleLootNoequip(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 2);
            }
        }
        return true;
    }
    static bool HandleLootEquip(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 3);
            }
        }
        return true;
    }
    static bool HandleLootGray(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 4);
            }
        }
        return true;
    }
    static bool HandleLootWhite(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 5);
            }
        }
        return true;
    }
    static bool HandleLootGreen(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 6);
            }
        }
        return true;
    }
    static bool HandleLootBlue(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 7);
            }
        }
        return true;
    }
    static bool HandleLootPurple(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                RBAC_QuestLoot(player, 8);
            }
        }
        return true;
    }
    static bool HandleLootCrafting(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
    ...
    static bool HandleXresetCommand(ChatHandler* handler, char const* /*args*/)
    {
        if (handler->GetSession())
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
    ...
    static bool HandleUnstuckCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession() && !handler->GetSession()->HasPermission(rbac::RBAC_PERM_COMMANDS_USE_UNSTUCK_WITH_ARGS))
        {
            ...
            if (Player* player = handler->GetSession()->GetPlayer()) {
                ...
                if (player->isDead())
                {
                    if (Spell * unstuck = player->FindCurrentSpellBySpellId(7355)) {
                        if (unstuck->getState() == SPELL_STATE_FINISHED)
                        {
                            player->ResurrectPlayer(0.1f);
    ...
    static bool HandleBankCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            if (Player* player = handler->GetSession()->GetPlayer()) {
                if (player->GetZoneId() == 33 && player->GetAreaId() == 2177) //Guru Arena
                    player->BroadcastMessage("You may not use this command in here");
                else
