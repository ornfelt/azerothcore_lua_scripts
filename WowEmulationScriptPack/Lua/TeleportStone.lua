...
function Stone.AddGossip(player, item, id)
	...
	if(id ~= MMENU)then--添加返回主菜单
		...
		if(player:GetGMRank()>=3)then--是GM
			--{MENU, "其他功能",		MMENU+0x10,		GOSSIP_ICON_TABARD},
			player:GossipMenuAddItem(GOSSIP_ICON_CHAT,"其他功能", 0, (MMENU+0x10)*0x100)
	...
	if(rowid== 0)then
		...
		if(v)then						--如果找到菜单项
			...
			elseif(mtype==TP)then	--传送
				...
				if(curMoney<TP_FEE) then
					player:SendBroadcastMessage("你身上的钱不够去"..map..",费用:"..(TP_FEE/10000).."G。")
					return
				end
				if(player:Teleport(mapid,x,y,z,o,TELE_TO_GM_MODE))then--传送
					Nplayer=GetPlayerByName(pname)--根据玩家名得到玩家
					if(Nplayer)then
						Nplayer:SendBroadcastMessage("已经到达 "..map..",费用:"..(TP_FEE/10000).."G。")
						Nplayer:ModifyMoney(-TP_FEE)--扣费
					end
				else
					print(">>Eluna Error: Teleport Stone : Teleport To "..mapid)
				end
