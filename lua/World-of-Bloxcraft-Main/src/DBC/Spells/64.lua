...
local speed = game.Lighting.Spells["64"].Speed.Value
local startPos = char.LeftHand.Position
local endPos = target.PrimaryPart.Position
local movingObject = game.ReplicatedStorage.SpellObjects["44"].VFX.Projectile:Clone()
...
local explosion = game.ReplicatedStorage.SpellObjects["44"].VFX.Explode.A0:Clone()
...
local function moveToTarget()
	...
	local function updateProjectile()
		...
		movingObject.CFrame = CFrame.lookAt(newPos, endPos):ToWorldSpace(CFrame.Angles(0, math.pi, 0))
		return true
	end
