...
function ControlModule:Root()
	self.rooted = true;
end

function ControlModule:Unroot()
	self.rooted = false;
end
...
function ControlModule:RotateCharacterToMouseDirection()
	local character = Players.LocalPlayer.Character
	if character then
		local humanoid = character:FindFirstChild("Humanoid")
		if humanoid then
			local cameraCFrame = Workspace.CurrentCamera.CFrame
			local lookVector = Vector3.new(cameraCFrame.LookVector.X, 0, cameraCFrame.LookVector.Z) -- remove Y component
			local rotation = math.atan2(-lookVector.X, -lookVector.Z)
			local targetCFrame = CFrame.new(character.PrimaryPart.Position) * CFrame.Angles(0, rotation, 0)

			-- Cancel the existing tween if it exists
			if self.tween then
				self.tween:Cancel()
				self.tween = nil
			end
			...
			local tweenInfo = TweenInfo.new(
				...
				0, -- RepeatCount (0 means the tween will not repeat)
				false, -- Reverses (should the tween reverse)
				0 -- DelayTime
			)

			-- Create the Tween
			self.tween = TweenService:Create(character.PrimaryPart, tweenInfo, {CFrame = targetCFrame})
...
local function calculateCharacterRelativeMoveVector(humanoid: Humanoid, characterRelativeMoveVector: Vector3): Vector3
	local character = humanoid.Parent
	if not character then
		return characterRelativeMoveVector
	end

	local characterCFrame = character.PrimaryPart.CFrame

	-- Extract the back vector components from the character's CFrame
	local c, s
	local _, _, _, R00, R01, R02, _, _, R12, _, _, R22 = characterCFrame:GetComponents()
	...
	-- Use these to calculate the new move vector
	local norm = math.sqrt(c*c + s*s)
	return Vector3.new(
		(c*characterRelativeMoveVector.X + s*characterRelativeMoveVector.Z)/norm,
		0,
		(c*characterRelativeMoveVector.Z - s*characterRelativeMoveVector.X)/norm
	)
end
...
function ControlModule:OnRenderStepped(dt)
	if self.activeController and self.activeController.enabled and self.humanoid then
		...
		if self.isRotatingToMouse then
			
			self:RotateCharacterToMouseDirection()
			cameraRelative = true
			
		else
			
			cameraRelative = false
			moveVector = calculateCharacterRelativeMoveVector(self.humanoid, moveVector)
		...
		if humanoidState ~= Enum.HumanoidStateType.Jumping and humanoidState ~= Enum.HumanoidStateType.Freefall and self.rooted == false then
