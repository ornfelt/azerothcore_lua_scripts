...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> gobjectCommandTable =
        {
            ...
            { "scale",        rbac::RBAC_PERM_COMMAND_GOBJECT_SCALE,        false, &HandleGameObjectScaleCommand,      ""       },
    ...
    static bool HandleGameObjectScaleCommand(ChatHandler* handler, char const* args)
    {
        ...
        char* NewScalestr = strtok(nullptr, " ");
        if (NewScalestr == NULL)
            return false;
        float NewScale = atof(NewScalestr);
        //set scale to new value ingame
        object->SetObjectScale(NewScale);

        //set scale in a persistant way
        uint32 guildLow = object->GetSpawnId();
        GameObjectAddon const *OldAddon = sObjectMgr->GetGameObjectAddon(guildLow);
        GameObjectAddon Addon;
        if (OldAddon != NULL)
            Addon = *OldAddon;
        else
        {
            Addon.invisibilityType = (InvisibilityType)0;
            Addon.InvisibilityValue = 0;
            Addon.ParentRotation = QuaternionData();
        }
        Addon.Scale = NewScale;
        sObjectMgr->UpdateGameObjectAddon(guildLow,&Addon);
