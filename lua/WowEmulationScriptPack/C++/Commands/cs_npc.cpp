...
public:
    ...
    static bool HandleNpcSetScaleCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 guildLow = creature->GetSpawnId();
        CreatureAddon const *OldAddon = sObjectMgr->GetCreatureAddon(guildLow);
        CreatureAddon Addon;
        if (OldAddon != NULL)
            Addon = *OldAddon;
        else
        ...
        sObjectMgr->UpdateCreatureAddon(guildLow, &Addon);
