...
public:
    ...
    struct codebox_passivesAI : public ScriptedAI
    {
        ...
        bool OnGossipHello(Player* player)
        {
            ...
            SendGossipMenuFor(player, 601021, me);
            return true;
        }

        bool OnGossipSelectCode(Player* player, uint32 menuId, uint32 gossipListId, const char* code)
        {
        ...
        void UpdateAI(uint32 diff)
        {
            if (uiAdATimer <= diff)
            {
                me->Say("Do you have a code to redeem? Step right up!", LANG_UNIVERSAL, NULL);
